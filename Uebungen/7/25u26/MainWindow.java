...
public class MainWindow extends Frame implements ActionListener, KeyListener {
	...
	public MainWindow(String title, Pong p) {
		...
		if(!this.pong.connectOnline()) {
			new WarnDialog(this, "Keine Verbindung zum Server!");
		}
	}
	 ...
	 */
	public void update() {
		this.score1.setText(Configuration.instance().getPlayer1Name() + ": " + Integer.toString(pong.getLinkPlayer(0).getScore()));
		this.score2.setText(Configuration.instance().getPlayer2Name() + ": " + Integer.toString(pong.getLinkPlayer(1).getScore()));
		this.display.setBallPosition(pong.getLinkBall().getXPosition(), pong.getLinkBall().getYPosition());
		this.display.setPaddle1Position(pong.getLinkPlayer(0).getLinkPaddle().getPosition());
		this.display.setPaddle2Position(pong.getLinkPlayer(1).getLinkPaddle().getPosition());
	}
}
