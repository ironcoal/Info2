...
public class Pong {
	...
	/**
	 * An instance of Online for server connection
	 */
	 ...
	 */
	public double calculatePaddleHitPosition(int player, double y) {
	...
	private void checkWin() {
		...
		if(players[0].getScore() >= conf.getWinScore() || players[1].getScore() >= conf.getWinScore()) {
			...
			if(hs != null) {
				...
				if(online.isLoggedIn() && players[0].getScore() >= conf.getWinScore()) {
					online.sendHighscore(hs);
				}
			}
			// stop the game
			this.gameAbort();
